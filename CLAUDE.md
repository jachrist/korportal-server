...
- Styrerom (`styrerom.html`/`js/styrerom.js`): samlet arbeidsflate for styret med faner (Medlemmer/Dokumenter/Oppgaver/Styrekalender). Hver fane er en `<iframe>` mot den eksisterende siden (`styre.html`, `styredokumenter.html`, `oppgaver.html`, `styremoter.html`) — lazy-lastet ved forste apning, deretter beholdt sa veksling er umiddelbar. Aktiv fane huskes i `localStorage['styrerom-tab']` og speiles i URL-hash (`#oppgaver`). De indre sidene skjuler sin egen topbar/meny nar de kjores i ramme (inline-skript setter `html.embedded`; CSS-regel i `style.css`), og verten synkroniserer tema til fanene. Tilgang: `initPage({ requiredRole: 'styre' })`.
